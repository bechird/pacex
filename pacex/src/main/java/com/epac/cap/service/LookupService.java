...
public class LookupService<L extends LookupItem> extends AbstractService{
	...
	@GET
	@Path("/prefGroupBySubject/{prefSubject}")
	public Response getPrefGroupBySubject( @PathParam("prefSubject") String prefSubject) {
		Map<String, String> constraintViolationsMessages = new HashMap<String, String>();
		String result = null;
		try{
			result = lookupHandler.getGroupBySubject(prefSubject);
		}catch (PersistenceException e) {
			constraintViolationsMessages.put("errors", "An error occurred while reading the pref group by subject!");
			InputResponseError inputResponseError = new InputResponseError(constraintViolationsMessages) ;
			return inputResponseError.createResponse();
		}
		return Response.ok(result).build();
