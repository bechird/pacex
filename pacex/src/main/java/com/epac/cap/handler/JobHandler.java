...
public class JobHandler {
		...
		try {
			...
			for(String stationCategoryId : requiredStations.keySet()){
				...
				if(theStation != null && theStation.getActiveFlag()){
					...
					// get whether sheet S0 is better or S# for press job based on waste values
					if(StationCategory.Categories.PRESS.toString().equals(stationCategoryId)){
						job.setBestSheetUsed(synchBestSheet(part, job.getQuantityNeeded()));
					}
	...
	private String synchBestSheet(Part part, Integer qty){
		String result = "S0";
		Preference lengthWhenSheetChangedValue = lookupDAO.read("LENGTHSHEETCHANGED", Preference.class);
		Float lengthWhenSheetChanged = (float) 10000;// in mm
		if(lengthWhenSheetChangedValue != null ){
			lengthWhenSheetChanged = Float.parseFloat(lengthWhenSheetChangedValue.getName());
		}
		if(part.getBestSheetWaste() != null){
			if(qty * part.getBestSheetWaste() < lengthWhenSheetChanged){
				result = part.getBestSheet();
			}
		}
		return result;
	}
	...
	public List<Job> getAvailableJobsForScheduling(String color, String paperType) throws PersistenceException{
		try{
			...
			String unit = "in";
			Preference pref = lookupDAO.read("UNITSYSTEM", Preference.class);
			if(pref != null && "FR".equals(pref.getName())){
				unit = "mm";
			}
			...
			for(Job j : result){
				...
				if(pr != null){
					StringBuilder sb = new StringBuilder(j.getJobType().getName());
					sb.append("/").append(pr.getLength().toString()).append(unit);
				  ...
				  ("O".equals(cascadeFlag) && theJob.getJobId().equals(job.getJobId()))){
					if(newQuantity - wasteQty > 0){
						...
						if(StationCategory.Categories.PRESS.toString().equals(theJob.getStationId())){
							newJob.setBestSheetUsed(synchBestSheet(pr, newJob.getQuantityNeeded()));
						}
					...
					if(StationCategory.Categories.PRESS.toString().equals(theJob.getStationId())){
						...
						theJob.setBestSheetUsed(synchBestSheet(pr, theJob.getQuantityNeeded()));
